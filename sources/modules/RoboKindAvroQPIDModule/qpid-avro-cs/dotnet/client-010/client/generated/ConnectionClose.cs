...
public sealed class ConnectionClose : Method {

    public const int TYPE = 267;
    ...
    private ConnectionCloseCode _ReplyCode;
    ...
    public ConnectionClose(ConnectionCloseCode ReplyCode, String ReplyText, params Option[] options) {
    ...
    public override void Dispatch<C>(C context, MethodDelegate<C> mdelegate) {
        mdelegate.ConnectionClose(context, this);
    }


    public bool HasReplyCode() {
    ...
    public override void Write(IEncoder enc)
    {
        ...
        if ((packing_flags & 256) != 0)
            enc.WriteUint16((short)_ReplyCode);
        if ((packing_flags & 512) != 0)
            enc.WriteStr8(_ReplyText);
    ...
    public override void Read(IDecoder dec)
    {
        ...
        if ((packing_flags & 256) != 0)
            _ReplyCode = ConnectionCloseCodeGetter.Get(dec.ReadUint16());
        if ((packing_flags & 512) != 0)
            _ReplyText = dec.ReadStr8();
    ...
    public override Dictionary<String,Object> Fields
    {
		get
		{
        	...
        	if ((packing_flags & 256) != 0)
            	result.Add("_ReplyCode", GetReplyCode());
        	if ((packing_flags & 512) != 0)
            	result.Add("_ReplyText", GetReplyText());
