...
public sealed class SessionAttach : Method {

    public const int TYPE = 513;
    ...
    public SessionAttach(byte[] Name, params Option[] options) {
        SetName(Name);

        for (int i=0; i < options.Length; i++) {
            switch (options[i]) {
            case Option.FORCE: packing_flags |= 512; break;
    ...
    public override void Dispatch<C>(C context, MethodDelegate<C> mdelegate) {
        mdelegate.SessionAttach(context, this);
    }
    ...
    public SessionAttach SetForce(bool value) {
    ...
    public override Dictionary<String,Object> Fields
    {
		get
		{
        	...
        	if ((packing_flags & 256) != 0)
            	result.Add("_Name", GetName());
        	if ((packing_flags & 512) != 0)
            	result.Add("_Force", GetForce());
