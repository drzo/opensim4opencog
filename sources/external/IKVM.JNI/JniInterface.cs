	...
	unsafe struct JNIEnv
	{
		...
		internal static jclass FindClass(JNIEnv* pEnv, byte* pszName)
		{
			try
			{
			    ...
			    var fnmcl = FindNativeMethodClassLoader(pEnv);
			    string dotname = name.Replace('/', '.');
                TypeWrapper wrapper;
			    try
			    {
                    wrapper = fnmcl.LoadClassByDottedNameFast(dotname);
			    }
			    catch (Exception e)
			    {
			        Warn("FindClass", e);
        ...
        static bool warnLevel = true;
        static public void SetWarnings(bool level)
        {
            warnLevel = level;
        }

        private static void Warn(string s, Exception exception)
        {
            string se = s + ": " + exception;
            ...
            else
            {
                ...
                Console.WriteLine(se + " ST: " + exception.StackTrace);
            }
