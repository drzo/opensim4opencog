/*
 *
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 *
 */

using System;
using org.apache.qpid.transport.codec;
using System.Collections.Generic;
using org.apache.qpid.transport.util;
using org.apache.qpid.transport.network;
using System.IO;

namespace org.apache.qpid.transport
{



public sealed class ConnectionStart : Method {

    public const int TYPE = 257;

    public override int GetStructType() {
        return TYPE;
    }

    public override int GetSizeWidth() {
        return 0;
    }

    public override int GetPackWidth() {
        return 2;
    }

    public override bool HasPayload() {
        return false;
    }

    public override byte EncodedTrack 
    {
       get{ return Frame.L1; }
       set { throw new NotImplementedException(); }
    }

    private int packing_flags = 0;
    private Dictionary<String,Object> _ServerProperties;
    private List<Object> _Mechanisms;
    private List<Object> _Locales;


    public ConnectionStart() {}


    public ConnectionStart(Dictionary<String,Object> ServerProperties, List<Object> Mechanisms, List<Object> Locales, params Option[] options) {
        SetServerProperties(ServerProperties);
        SetMechanisms(Mechanisms);
        SetLocales(Locales);

        for (int i=0; i < options.Length; i++) {
            switch (options[i]) {
            case Option.SYNC: Sync = true; break;
            case Option.BATCH: Batch = true; break;
            case Option.NONE: break;
            default: throw new Exception("invalid option: " + options[i]);
            }
        }

    }

    public override void Dispatch<C>(C context, MethodDelegate<C> mdelegate) {
        mdelegate.ConnectionStart(context, this);
    }


    public bool HasServerProperties() {
        return (packing_flags & 256) != 0;
    }

    public ConnectionStart ClearServerProperties() {
        packing_flags = (byte) (packing_flags & ~256);       

        Dirty = true;
        return this;
    }

    public Dictionary<String,Object> GetServerProperties() {
        return _ServerProperties;
    }

    public ConnectionStart SetServerProperties(Dictionary<String,Object> value) {
        _ServerProperties = value;
        packing_flags |=  256;
        Dirty = true;
        return this;
    }


    public bool HasMechanisms() {
        return (packing_flags & 512) != 0;
    }

    public ConnectionStart ClearMechanisms() {
        packing_flags = (byte) (packing_flags & ~512);       

        Dirty = true;
        return this;
    }

    public List<Object> GetMechanisms() {
        return _Mechanisms;
    }

    public ConnectionStart SetMechanisms(List<Object> value) {
        _Mechanisms = value;
        packing_flags |=  512;
        Dirty = true;
        return this;
    }


    public bool HasLocales() {
        return (packing_flags & 1024) != 0;
    }

    public ConnectionStart ClearLocales() {
        packing_flags = (byte) (packing_flags & ~1024);       

        Dirty = true;
        return this;
    }

    public List<Object> GetLocales() {
        return _Locales;
    }

    public ConnectionStart SetLocales(List<Object> value) {
        _Locales = value;
        packing_flags |=  1024;
        Dirty = true;
        return this;
    }





    public override void Write(IEncoder enc)
    {
        enc.WriteUint16(packing_flags);
        if ((packing_flags & 256) != 0)
            enc.WriteMap(_ServerProperties);
        if ((packing_flags & 512) != 0)
            enc.WriteArray(_Mechanisms);
        if ((packing_flags & 1024) != 0)
            enc.WriteArray(_Locales);

    }

    public override void Read(IDecoder dec)
    {
        packing_flags = (int) dec.ReadUint16();
        if ((packing_flags & 256) != 0)
            _ServerProperties = dec.ReadMap();
        if ((packing_flags & 512) != 0)
            _Mechanisms = dec.ReadArray();
        if ((packing_flags & 1024) != 0)
            _Locales = dec.ReadArray();

    }

    public override Dictionary<String,Object> Fields
    {
		get
		{
			Dictionary<String,Object> result = new Dictionary<String,Object>();

        	if ((packing_flags & 256) != 0)
            	result.Add("_ServerProperties", GetServerProperties());
        	if ((packing_flags & 512) != 0)
            	result.Add("_Mechanisms", GetMechanisms());
        	if ((packing_flags & 1024) != 0)
            	result.Add("_Locales", GetLocales());

			return result;
        }
    }

}
}
