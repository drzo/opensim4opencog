...
public sealed class QueueDelete : Method {

    public const int TYPE = 2050;
    ...
    public QueueDelete(String Queue, params Option[] options) {
        SetQueue(Queue);

        for (int i=0; i < options.Length; i++) {
            switch (options[i]) {
            case Option.IF_UNUSED: packing_flags |= 512; break;
            case Option.IF_EMPTY: packing_flags |= 1024; break;
    ...
    public override void Dispatch<C>(C context, MethodDelegate<C> mdelegate) {
        mdelegate.QueueDelete(context, this);
    }
    ...
    public bool GetIfUnused() {
        return HasIfUnused();
    }

    public QueueDelete SetIfUnused(bool value) {
    ...
    public QueueDelete SetIfEmpty(bool value) {
    ...
    public override Dictionary<String,Object> Fields
    {
		get
		{
        	...
        	if ((packing_flags & 512) != 0)
            	result.Add("_IfUnused", GetIfUnused());
        	if ((packing_flags & 1024) != 0)
            	result.Add("_IfEmpty", GetIfEmpty());
