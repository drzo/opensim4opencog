...
public sealed class DtxEnd : Method {
    ...
    public DtxEnd(Xid Xid, params Option[] options) {
        SetXid(Xid);

        for (int i=0; i < options.Length; i++) {
            switch (options[i]) {
            case Option.FAIL: packing_flags |= 512; break;
            case Option.SUSPEND: packing_flags |= 1024; break;
    ...
    public DtxEnd SetFail(bool value) {
    ...
    public DtxEnd SetSuspend(bool value) {
    ...
    public override Dictionary<String,Object> Fields
    {
		get
		{
        	...
        	if ((packing_flags & 512) != 0)
            	result.Add("_Fail", GetFail());
        	if ((packing_flags & 1024) != 0)
            	result.Add("_Suspend", GetSuspend());
