...
public sealed class ConnectionOpen : Method {

    public const int TYPE = 263;
    ...
    private List<Object> _Capabilities;


    public ConnectionOpen() {}


    public ConnectionOpen(String VirtualHost, List<Object> Capabilities, params Option[] options) {
        SetVirtualHost(VirtualHost);
        SetCapabilities(Capabilities);

        for (int i=0; i < options.Length; i++) {
            switch (options[i]) {
            case Option.INSIST: packing_flags |= 1024; break;
    ...
    public override void Dispatch<C>(C context, MethodDelegate<C> mdelegate) {
        mdelegate.ConnectionOpen(context, this);
    }


    public bool HasVirtualHost() {
    ...
    public ConnectionOpen SetCapabilities(List<Object> value) {
        _Capabilities = value;
        packing_flags |=  512;
        Dirty = true;
        return this;
    ...
    public bool GetInsist() {
        return HasInsist();
    }

    public ConnectionOpen SetInsist(bool value) {
    ...
    public override void Write(IEncoder enc)
    {
        ...
        if ((packing_flags & 256) != 0)
            enc.WriteStr8(_VirtualHost);
        if ((packing_flags & 512) != 0)
            enc.WriteArray(_Capabilities);
    ...
    public override void Read(IDecoder dec)
    {
        ...
        if ((packing_flags & 256) != 0)
            _VirtualHost = dec.ReadStr8();
        if ((packing_flags & 512) != 0)
            _Capabilities = dec.ReadArray();
    ...
    public override Dictionary<String,Object> Fields
    {
		get
		{
        	...
        	if ((packing_flags & 256) != 0)
            	result.Add("_VirtualHost", GetVirtualHost());
        	if ((packing_flags & 512) != 0)
            	result.Add("_Capabilities", GetCapabilities());
        	if ((packing_flags & 1024) != 0)
            	result.Add("_Insist", GetInsist());
