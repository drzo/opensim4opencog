    ...
    public class DownloadTerrainCommand : Command, RegionMasterCommand, AsynchronousCommand
    {
        ...
        public override CmdResult ExecuteRequest(CmdRequest args)
        {
            ...
            EventHandler<InitiateDownloadEventArgs> initiateDownloadDelegate =
                delegate(object sender, InitiateDownloadEventArgs e) { Client.Assets.RequestAssetXfer(e.SimFileName, false, false, UUID.Zero, AssetType.Unknown, false); };
        ...
        private void Assets_XferReceived(object sender, XferReceivedEventArgs e)
        {
            if (e.Xfer.Success)
            {
                ...
                result.AppendFormat("Terrain file {0} ({1} bytes) downloaded successfully, written to {2}",
                                    e.Xfer.Filename, e.Xfer.Size, fileName);
