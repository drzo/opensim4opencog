...
(def (on-chat agent message)
  (block
    ...
    ; (thisClient.ExecuteCommand (@"{0}" (str message)) )
     ...
     (cloj set mymsg (str message))
    ; (cloj set mymsgList (into nil (cloj mymsg.Split(" ") )))
     
    ; (when (member mymsgList wamo )
    ;  (thisClient.ExecuteCommand "say I just saw wamo")
    ;  )
      
     (when (>= (cloj mymsg.IndexOf "hello" ) 0) 
        (thisClient.ExecuteCommand (@"say Hello there {0} !!!" (str agent) ) )
     )
      (when (>= (cloj mymsg.IndexOf "use" ) 0) 
        (thisClient.ExecuteCommand (@"{0}" (str message)) )
     )
     (when (>= (cloj mymsg.IndexOf "follow" ) 0) 
        (thisClient.ExecuteCommand (@"{0}" (str message)) )
     )
     (when (>= (cloj mymsg.IndexOf "cogbot" ) 0)
       (block 
        (cloj set mycommand (str message))
        (cloj set mycommand1 (cloj mycommand.Replace "cogbot " ""))
