...
set COMMAND=ikvm %QPID_OPTS% %SYSTEM_PROPS% -cp "%CLASSPATH%;%MODULE_JARS%" org.apache.qpid.server.Main %QPID_ARGS%
...
@rem ikvm  -Damqj.logging.level=info "-DQPID_HOME=C:\development\opensim4opencog\HR\apollomind\tools\qpid-broker-0.12" "-DQPID_WORK=" -cp "C:\development\opensim4opencog\HR\apollomind\tools\qpid-broker-0.12\lib\qpid-all.jar;" org.apache.qpid.server.Main

@rem ikvm  -Damqj.logging.level=info "-DQPID_HOME=..\" "-DQPID_WORK=" -cp qpid-all.jar org.apache.qpid.server.Main

@rem in the next hour I am making sure Cogbot starts the JMX/QPID/AVRO server if not running
:end
