...
namespace LAIR.ResourceAPIs.WordNet
{
    ...
    public class WordNetEngine
    {
        ...
        private Dictionary<POS, BinarySearchTextStream> _posWordIndexSearcher;     // disk-based word index searchers
        ...
        public Dictionary<POS, Set<string>> AllWords
        {
            get
            {
                ...
                if (_inMemory)
                    ...
                    foreach (POS pos in _posWordIndexSearcher.Keys)
                    {
                        Set<string> words = new Set<string>();
                        StreamReader indexFile = _posWordIndexSearcher[pos].Stream;
        ...
        public WordNetEngine(string wordNetDirectory, bool inMemory)
        {
            ...
            {
                // open binary searches for index files
                _posWordIndexSearcher = new Dictionary<POS, BinarySearchTextStream>();
                foreach (string indexPath in indexPaths)
                {
                    POS pos = GetFilePOS(indexPath);
                    ...
                    _posWordIndexSearcher.Add(pos, searchStream);
                }
        ...
        public Set<SynSet> GetSynSets(string word, params POS[] posRestriction)
        {
            ...
            foreach (POS pos in posSet)
                ...
                else
                {
                    ...
                    string indexLine = _posWordIndexSearcher[pos].Search(word);
        ...
        public void Close()
        {
            ...
            else
            {
                ...
                foreach (BinarySearchTextStream stream in _posWordIndexSearcher.Values)
                    stream.Close();

                // close all data files
                foreach (StreamReader stream in _posSynSetDataFile.Values)
                    stream.Close();
            }
        }
    }
