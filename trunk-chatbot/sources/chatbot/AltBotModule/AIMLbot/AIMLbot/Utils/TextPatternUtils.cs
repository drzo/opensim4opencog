...
namespace AltAIMLbot.Utils
{
    ...
    public class TextPatternUtils : StaticXMLUtils
    {
        ...
        public static bool IsFalse(Unifiable tf)
        {
            ...
            tf = tf.ToLower();
            return tf.Contains("no") || tf.Contains("alse");
        }

        public static bool IsNullOrEmpty(Object name)
        {
            if (name is Unifiable) return string.IsNullOrEmpty(((Unifiable)name));
...
//            if (ReferenceEquals(name, Unifiable.NULL)) return true;
  ...
  //              s = ((Unifiable)name).SpecialName;
        ...
        public static bool IsUnknown(object unifiable)
        {
            ...
            string ss = unifiable.ToString().ToLower();
            ...
            {
                if (!IsWildCard(unifiable)) return false;
                return true;
            }
            return false;
            //switch (s)
        ...
        {
            return cond == null || cond == "*" || cond == Unifiable.Empty;
        }
