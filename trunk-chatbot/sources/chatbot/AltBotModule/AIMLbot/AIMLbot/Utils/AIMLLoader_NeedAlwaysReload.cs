...
namespace AltAIMLbot.Utils
{
    ...
    /// </summary>           
    [StructToString(true)]
    public class AIMLLoader_NeedAlwaysReload : XmlNodeEvaluatorImpl, AIMLLoader
    {
        ...
        internal AIMLLoader_NeedAlwaysReload(AltBot bot)
        {
            this.bot = bot;
        }

        public AIMLLoader_NeedAlwaysReload(AltBot bot, Request request)
        {
            request.loader = this;
            this.bot = bot;
        ...
        /// <param name="node"> the XML node containing the category </param>
        /// <param name="filename"> the file from which this category was taken </param>
        public long processCategory(XmlNode node, string filename)
        {
            var pc = processCategory(node, node.ParentNode, loadOpts, loadOpts.AdditionalPreconditions);
            if (pc == null) return 1;
            return pc.Count;
        }
        ...
        public long loadAIMLString(string docString)
        {
            //            RProcessor0.ReloadHooks.Add(() => loadAIMLFile0(path, loadOpts));
            if (SMLTranslator.isSML(docString))
            {
                docString = SMLTranslator.sml2xml(docString);
            }
