...
namespace AltAIMLbot.Utils
{
    internal class TestCaseRunner : XmlNodeEvaluatorImpl
    {
        ...
        private XmlNode RunTest(Request request, XmlNode src, OutputDelegate outputdelegate, out bool m)
        {
            ...
            string tcdesc = StaticXMLUtils.FindNodeOrAttrib(src, "Description", null);
            string input = StaticXMLUtils.FindNodeOrAttrib(src, "Input", null);
            if (input == null)
            {
                outputdelegate("ERROR cannot find 'Input' in '" + src.OuterXml + "'");
                m = false;
                return StaticXMLUtils.getNodeAndSetSibling(
                    "<template type=\"error\">ERROR cannot find 'Input' in '" + src.OuterXml + "'</template>", true,
                    false, src);
            }
            string userID = StaticXMLUtils.FindNodeOrAttrib(src, "UserId,UserName", () => user.UserID);

            const string MISSING_EXPECTED_ANSWER = "ExpectedKeywords";
            var matchTheseToPass = new List<string>();
            string expectedAnswer = StaticXMLUtils.FindNodeOrAttrib(src, "ExpectedAnswer", () => MISSING_EXPECTED_ANSWER);
            expectedAnswer = Fudge(expectedAnswer);
            if (expectedAnswer == MISSING_EXPECTED_ANSWER)
            {
                var nodes = StaticXMLUtils.FindNodes("ExpectedKeywords", src);
                ...
                else
                {
                    ...
                    {
                        string v = StaticXMLUtils.InnerXmlText(list);
            ...
            try
            {
                var requestToBot = new Request(input, user, robot);
                ...
                {
                    if (!Matches(resp, s, TextPatternUtils.FindNodeOrAttrib(src, "MatchType,Match", null)))
                    {
                        m = false;
                    }
                    else
        ...
        private XmlNode GetMessage(XmlNode src, string attrs, string msg)
        {
            ...
            return StaticXMLUtils.getNodeAndSetSiblingNode("<template " + attrs + " >" + attrs + " " + msg + "</template>", src);
        }
        ...
        {
            expectedAnswer = TextPatternUtils.CleanPunct(StaticXMLUtils.CleanWhitepaces(expectedAnswer));
            ...
            {
                expectedAnswer = expectedAnswer.Replace(".", " ");
                return StaticXMLUtils.CleanWhitepaces(expectedAnswer);
            }
            expectedAnswer = expectedAnswer.Replace(".", " ");
            return StaticXMLUtils.CleanWhitepaces(expectedAnswer);
        }
