...
noun_template(Noun,TypeV,V,`P,
      [slot(poss,TypeO,O,Os,index80)|Slots]) :-
...
index_slot(index80,I,I).
index_slot(free,_,unit).
index_slot(apply,_,apply).
index_slot(comparator,_,comparator).

index_args(det(the(plu)),unit,I,set(I),index80(I)) :- !.
index_args(int_det(X),index80(I),_,int_det(I,X),unit) :- !.
