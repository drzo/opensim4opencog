...
%%:- load_foreign_library('jpl.dll').
%%:- load_foreign_library('swicli.dll').

%:-load_foreign_library('jpl').
...
simObject(Ele):-cliGet('cogbot.Listeners.WorldObjects','SimObjects',Obj),cliCollection(Obj,Ele).
...
onSimEvent(_A,_B,_C):-!.
onSimEvent(A,B,C):-cliToStringS(onSimEvent(A,B,C),AS),writeq(AS),nl.
