...
using CONTEXT = RTParser.Prolog.PrologEngine;
    ...
    */
    ...
    public class Ext
    {
        ...
        static public ICollection CollectionFromTerm(Term term/*CONTEXT*/)
        {
            ...
            if (term.IsString)
            {
                return term.Functor.ToCharArray();
            }
            if (term.IsList)
            {
            ...
            if (term.IsAtom)
            {
                return (ICollection)AtomAsObject(term.FunctorU);
            }
            if (term.IsCompound)
            {
                return (ICollection)CompoundToObject(term.FunctorU, term.Args/*ctx*/);
            }
            if (term.IsNumber)
            {
                return (ICollection)term.ExprValue.AsObject;
        ...
        static public object ObjectFromTerm(Term term/*CONTEXT*/)
        {
            ...
            if (term.IsNumber)
            {
                Term.OperandValue expv = term.ExprValue;
                string value = term.Functor;
                if (value.Contains("."))
                {
                    return (double)expv.AsNumber;
                }
                return expv.AsInteger;
            }
            if (term.IsAtom)
            {
                return AtomAsObject(term.FunctorU);
            }
            if (term.IsCompound)
            {
        ...
        {
            Term n = Term.NULLLIST;
            int i = l.Count;
            while (i-- > 0)
            {
                n = new ListTerm(ObjectToTerm(l[i]), n);
            }
            return n;
        }
        ...
        static public bool ObjectUnify(object r, Term t2/*CONTEXT*/)
        {
            if (t2.IsVar)
            {
                return t2.Unify(ObjectToTerm(r),varStack);
            }
            if (t2.IsCompound)
            {
                if (t2.FunctorU == "{}")
                {
                    if (r is Term) return t2.Arg(0).Unify((Term)r,varStack);
                    return t2.Arg(0).Unify(ObjectToTerm(r), varStack);
                }
            }
            if (r == null) return true;
            Type t = r.GetType();

            if (t2.ExprValue.AsObject.Equals(r)) return true;
            return true;
        }

        static public Term ObjectToTerm(object o)
        {
            if (o is string) return new Term((string)o);
            if (o is bool) return (o is bool) ? Term.TRUE : Term.FALSE;
            return new Term(o, (Type)null);
        }
        ...
        private static bool CanConvert(Term term, Type type, ref object argarray, out object value/*CONTEXT*/)
        {
            ...
            if (DoesConvert(type, term._functor, out value)) return true;
